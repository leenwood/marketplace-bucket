...
func newTestUseCase(r *mockRepo) *usecase.CartUseCase {
	return usecase.NewCart(r, slog.Default())
}
...
func TestCartUseCase_AddItem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cart, err := uc.AddItem(context.Background(), tt.userID, tt.item)
			if tt.wantErr != nil {
...
func TestCartUseCase_RemoveItem(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cart, err := uc.RemoveItem(context.Background(), tt.userID, tt.productID)
			if tt.wantErr != nil {
				require.ErrorIs(t, err, tt.wantErr)
				assert.Nil(t, cart)
			} else {
...
func TestCartUseCase_UpdateQuantity(t *testing.T) {
	tests := []struct {
		name      string
		userID    string
		productID string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cart, err := uc.UpdateQuantity(context.Background(), tt.userID, tt.productID, tt.quantity)
			if tt.wantErr != nil {
				require.ErrorIs(t, err, tt.wantErr)
				assert.Nil(t, cart)
			} else {
...
func TestCartUseCase_GetCart(t *testing.T) {
	t.Run("returns existing cart", func(t *testing.T) {
		...
		cart, err := newTestUseCase(r).GetCart(context.Background(), "u1")
		require.NoError(t, err)
...
func TestCartUseCase_ClearCart(t *testing.T) {
	t.Run("deletes cart", func(t *testing.T) {
		...
		require.NoError(t, newTestUseCase(r).ClearCart(context.Background(), "u1"))
		r.AssertExpectations(t)
	})

	t.Run("empty user ID", func(t *testing.T) {
		r := &mockRepo{}
		err := newTestUseCase(r).ClearCart(context.Background(), "")
